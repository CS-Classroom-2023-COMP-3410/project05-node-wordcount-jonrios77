...
function getWordCounts(content) {
    ...
    for (const word of words) {
        // If the word exists in our object, increment it; otherwise, set it to 1
        if (wordCount[word]) {
            wordCount[word]++;
        } else {
            wordCount[word] = 1;
...
function colorWord(word, count) {
    ...
    if (count === 1) {
        ...
        // Fallback for any words with 0 counts (though rare in this context)
        return word;
    }}
...
function printColoredLines(content, wordCount) {
    ...
    for (const line of lines) {
        const coloredLine = line.split(/\W+/).map(word => {
            ...
            if (word) {
                // Look up the count using lowercase to match your wordCount object
                const count = wordCount[word.toLowerCase()] || 0;
                return colorWord(word, count);
            }
            // If the word is empty (like at the end of a line), return an empty string
            return ""; 
        }).join(' ');
